...
object TSSInterface  {
  ...
  //
  //  def getSeriesPeriodogramsAndPcaCoefsWithVarExplained(tss: TSS, sizePeriodogram: Int = 10,
  //                                                       nPcaAxisMax: Int = 5,
  //                                                       thresholdVarExplained: Double = 0.99)(implicit ss: SparkSession) = {
  //
  //    val periodogramAndSeries = getPeriodogramsAndSeries(tss, sizePeriodogram)
  ...
  //    val series = periodogramAndSeries.map(row => (row._1, row._2, BzDenseVector(row._4.toArray)))
  //
  //    val periodogramByRow: RDD[(Int, Array[BzDenseVector[Double]])] = fromCellDistributionToRowDistribution(periodogramAndSeries.map(row => (row._1, row._2, BzDenseVector(row._3.toArray))))
  //    val seriesByRow: RDD[(Int, Array[BzDenseVector[Double]])] = fromCellDistributionToRowDistribution(series)
  //
  //    val p = periodogramByRow.take(1).head._2.length
  //    val dummyPartition = List.fill(p)(0)
  //    val partitionPerColBc: Broadcast[BzDenseVector[Int]] = ss.sparkContext.broadcast(BzDenseVector(dummyPartition: _*))
  ...
  //      verbose = true)
  //
  //    val (loadings, eigenValues) = getLoadingsAndEigenValues(covMat.flatten.head, nPcaAxisMax, thresholdVarExplained)
  //    val pcaCoefs = periodograms.map(r => (r._1, r._2,
  //      loadings * (BzDenseVector(r._3.toArray) - meanList.flatten.head)))
  //
  //    val pcaCoefByRow = fromCellDistributionToRowDistribution(pcaCoefs)
  //
  //    (seriesByRow, periodogramByRow, pcaCoefByRow, eigenValues)
  //  }
  ...
  def getPcaCoefs(tss:TSS)(implicit ss: SparkSession) = {
